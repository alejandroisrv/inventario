...
foreach ($ventasGeneral as $ventaGeneral) {
    ...
    $estadoPedido= ($ventaGeneral['estado']==1) ? 'Entregado' : 'No entregado/rechazado' ;
    if(($metodoPago!="todos")){
      if($ventaGeneral['metodoPago']==$metodoPago){
        if(($metodoVenta==$ventaGeneral['metodoVenta']) || ($metodoVenta=="todos") || ($metodoVenta=="delivery")){
            $tablaInterna.="
                  <br>
