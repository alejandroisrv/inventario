...
class ventas
{
  ...
  {
    if(($desde===null ) && ($hasta===null)){
      $sqlAdmin="SELECT venta.producto as producto,venta.cantidad,productos.precioCosto,venta.precio as precio FROM venta INNER JOIN productos on venta.idproducto= productos.idproducto WHERE fecha BETWEEN now() - interval 7 day AND now() ORDER BY venta.producto ASC";
    }else {
      $sqlAdmin="SELECT venta.producto as producto,venta.cantidad,productos.precioCosto,venta.precio as precio FROM venta INNER JOIN productos on venta.idproducto= productos.idproducto WHERE fecha BETWEEN '$desde' AND '$hasta' ORDER BY venta.producto ASC";
    }
